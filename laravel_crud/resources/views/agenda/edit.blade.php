...
<main>
    ...
    <form class="ml-5 mt-5" method="POST" enctype="multipart/form-data" action="{{ route('agenda.update', $contact) }}">
        ...
        <label for="address"> Contact address:
            <input class="border-2 border-solid border-gray-100 rounded-full px-2" type="text" name="address"
                   value="{{old('address', $contact->address)}}" placeholder="Address 123, street"/>
        </label><br>
        <button
